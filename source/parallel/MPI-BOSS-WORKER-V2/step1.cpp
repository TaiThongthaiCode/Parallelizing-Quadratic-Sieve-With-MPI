...
int main(int argc, char *argv[]){
    ...
    //Write primes and solutions a, b suh that a^2 = b^2 = N (mod p) into
    //a text file; also write in nuber of primes
    ofstream fb;
...
int getprimes(int l, mpz_t N, prime_element * primes, int fbs){
    ...
    truth[0] = false;
    truth[1] = false;
    //check all i less than sqrt(l), when something is prime, make all
    //multiples not prime (starting from squared, as others are arleady covered)
    for (int i = 0; i*i < l; i++){
        if (truth[i]){
